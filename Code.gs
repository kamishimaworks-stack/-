/**
 * AI建築見積システム v10.0
 * Code.gs - 完全版 (Phase 4: 発注書管理・再発行機能追加)
 */
...
function include(filename) {
  try {
    ...
    console.warn("Template include failed: " + filename);
    return ""; 
  }
}
...
function deleteRowsById(sheet, targetId) {
  ...
  for (let i = 1; i < data.length; i++) {
    const rowId = String(data[i][0]); if (rowId !== "") currentId = rowId;
    if (currentId === targetId) rowsToDelete.push(i + 1);
  }
...
function apiListDriveFiles() {
  ...
  try {
    ...
    const result = [];
    while (files.hasNext()) { 
        const f = files.next(); const m = f.getMimeType(); 
    ...
    return JSON.stringify(result.sort((a,b)=>new Date(b.updated)-new Date(a.updated)).slice(0, 30));
...
function apiGetProjects() {
  ...
  return JSON.stringify(Object.values(projectMap).sort((a, b) => new Date(b.date) - new Date(a.date)));
}
...
function apiGetActiveProjectsList() {
  ...
  return JSON.stringify(projects.reverse());
}
  ...
  try {
    ...
    if (oData.length > 1) {
        for (let i = 1; i < oData.length; i++) {
            if (String(oData[i][3]) === saveId) { rowsToDelete.push(i + 1); }
        }
        rowsToDelete.reverse().forEach(r => orderSheet.deleteRow(r));
    }
    ...
    if (estimateData.items) {
      estimateData.items.forEach((item) => {
          if (item.vendor && (Number(item.cost) > 0 || Number(item.qty) > 0)) {
              ...
              orderValues.push([
                  orderId, saveTimestamp, item.vendor, saveId, 
                  item.category, item.product, item.spec, item.qty, item.unit, item.cost, Math.round(item.qty * item.cost), 
    ...
    if (orderValues.length > 0) {
        orderSheet.getRange(orderSheet.getLastRow() + 1, 1, orderValues.length, 16).setValues(orderValues.map(r => { while(r.length < 16) r.push(""); return r; }));
    }
...
function apiSaveAndCreateEstimatePdf(jsonData) {
  ...
  try {
    ...
    if (CONFIG.saveFolder) { try { folder = DriveApp.getFolderById(CONFIG.saveFolder); } catch (e) {} }
...
function apiUpdateInvoiceStatus(id, newStatus) {
  ...
  let found = false;
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]) === id) { sheet.getRange(i + 1, 2).setValue(newStatus); found = true; break; }
  }
...
function apiGetAnalysisData(year) {
  ...
  return JSON.stringify({ monthly: monthlyStats, ranking: clientRanking });
}
